"use client";

import React from 'react';
import { motion } from 'framer-motion';
import Image from 'next/image';

const technologies = [
    {
        title: "국제 규격 및 표준 인터페이스 지원",
        desc: "NMEA 0183, NMEA 2000, IEC 61162 등 선박용 데이터 통신 규격 지원. 향후 e-Navigation, 자율운항·VTS 연계에 필요한 표준 프로토콜을 확장 적용할 수 있는 구조.",
    },
    {
        title: "센서 융합 및 CPA/TCPA 분석 엔진",
        desc: "AIS·레이더·GPS·IMU 데이터를 융합하여 자선과 타선의 CPA, TCPA를 실시간 계산. 위험도에 따라 색상·음향·팝업 등 다양한 경보 방식 제공.",
    },
    {
        title: "고신뢰성 하드웨어 및 선박 환경 대응 설계",
        desc: "장시간 연속 운전과 전원 변동(발전기·배터리 환경)에 대응하는 전원 설계. 밝은 햇빛·야간 운항 모두에서 가독성이 뛰어난 디스플레이 채택.",
    },
    {
        title: "사용 편의성을 고려한 UI/UX",
        desc: "해상 라인업에 맞게 버튼 크기, 색상 대비, 야간·주간 모드를 세심하게 설계. 실제 선장·조타수의 피드백을 반영한 현장 중심 UI/UX를 지속 개선합니다.",
    },
];

export function KeyTechnologiesSection() {
    return (
        <section id="technology" className="w-full bg-white py-20 md:py-28">
            <div className="w-full max-w-[1920px] mx-auto px-8 md:px-14 lg:px-20">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">

                    <motion.div
                        initial={{ opacity: 0, y: 16 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.7 }}
                    >
                        <p className="section-label mb-5">Technology</p>
                        <h2 className="text-4xl md:text-5xl font-semibold text-[#0A1628] tracking-tight mb-4">주요 기술</h2>
                        <div className="w-12 h-[2px] bg-[#B8965F] mb-10" />

                        <div className="space-y-8">
                            {technologies.map((tech, idx) => (
                                <div key={idx} className="flex gap-5">
                                    <span className="text-xs font-semibold text-[#B8965F] tabular-nums mt-1 w-5 flex-shrink-0">
                                        {String(idx + 1).padStart(2, '0')}
                                    </span>
                                    <div>
                                        <h3 className="text-base font-semibold text-[#0A1628] mb-2">{tech.title}</h3>
                                        <p className="text-sm text-[#4B5C73] leading-relaxed">{tech.desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </motion.div>

                    <motion.div
                        initial={{ opacity: 0, x: 16 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.7, delay: 0.15 }}
                        className="lg:sticky lg:top-32"
                    >
                        <div className="rounded-2xl overflow-hidden border border-[#E5E8EF]">
                            <Image
                                src="/images/Ai.png"
                                alt="Navigation Technology"
                                className="w-full object-cover"
                                width={1000}
                                height={800}
                            />
                        </div>
                    </motion.div>
                </div>
            </div>
        </section>
    );
}
